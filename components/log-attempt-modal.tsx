...
export function LogAttemptModal({ open, onOpenChange, lead, onAttemptLogged }: LogAttemptModalProps) {
  ...
  const handleSave = async () => {
    ...
    if (data) {
      ...
      // Emit workflow event: outcome logged
      emitWorkflowEvent({
        type: "outcome_logged",
        leadId: lead.id,
        payload: { outcome, why: why || undefined },
        timestamp: new Date().toISOString(),
      })
      ...
      if (lead) {
        ...
        const stagePromotion: Record<string, string> = {
          [OUTCOMES.MEETING_SET.value]: "Meeting Booked",
          [OUTCOMES.DM_SOME_INTEREST.value]: "Interested",
        }
        const dropOutcome = outcome === OUTCOMES.DM_NO_INTEREST.value && (why === "Targeting" || why === "Value" || why === "Trust")
          ...
          supabase
            ...
            .then(({ error: stageError }) => {
              if (stageError) console.warn("[auto-stage] Skipped:", stageError.message)
                ...
                // Emit stage_change event for the auto-promotion
                emitWorkflowEvent({
                  type: "stage_change",
                  leadId: lead.id,
                  payload: { from_stage: lead.stage, to_stage: newStage },
                  timestamp: new Date().toISOString(),
                })
              }
            })
