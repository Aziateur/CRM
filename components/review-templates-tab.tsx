...
function FieldEditorRow({
    ...
    const options = (field.config.options ?? []) as { value: string; label: string; color?: string; score?: number }[]
    ...
    const updateOption = (idx: number, patch: Partial<{ value: string; label: string; color?: string; score?: number }>) => {
        ...
        <div className="border rounded-lg p-3 space-y-3">
            ...
            {expanded && (
                <div className="ml-6 space-y-3">
                            ...
                            {options.map((opt, idx) => (
                                <div key={idx} className="flex items-center gap-2">
                                    ...
                                    <Input
                                        ...
                                        className="h-7 text-xs w-16 text-center tabular-nums"
                                        title="Score weight for ranking"
                                    />
                                    <button
                    ...
                    {/* Checkbox-specific: description + score */}
                    {field.fieldType === "checkbox" && (
                        <div className="space-y-2">
                            ...
                            <div>
                                <Label className="text-xs">Score when checked</Label>
                                <Input
                                    type="number"
                                    value={(field.config.checkedScore as number) ?? 0}
                                    onChange={(e) =>
                                        onUpdate({ config: { ...field.config, checkedScore: Number(e.target.value) || 0 } })
                                    }
                                    placeholder="0"
