...
export function LeadImport({ fieldDefinitions, onImported }: LeadImportProps) {
  ...
  const handleImport = async () => {
    ...
    if (!projectId) {
      toast({ variant: "destructive", title: "No project selected", description: "Please log in and select a project before importing leads." })
      return
    }
    ...
    for (let i = 0; i < insertRows.length; i += chunkSize) {
      ...
      if (error) {
        ...
        // Auto-create a default contact for leads with a phone number
