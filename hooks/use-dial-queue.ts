...
export interface QueueItem {
  ...
  /** Numeric priority score (higher = more urgent). Used for weighted sorting. */
  priority: number
  /** Extensible metadata for future features (KB context, scoring signals, etc.) */
...
export function useDialQueue(
  ...
  const queue = useMemo(() => {
      ...
      for (const lead of dialable) {
        ...
        if (leadTasks && leadTasks.length > 0) {
          ...
          items.push({
            ...
            task: urgentTask,
            source: "followups",
            priority: isOverdue ? 100 + daysOverdue : dueDate < todayEnd ? 90 : 60,
          })
          continue
        }
    ...
    for (const lead of dialable) {
      ...
      if (!lastAttempt) {
        fresh.push({ lead, reason: "Fresh lead", task: null, source: "new", priority: 50 })
      } else if (
        !hasPendingTask &&
        (lastAttempt.nextAction === "Call again" || lastAttempt.nextAction === "Follow up")
      ) {
