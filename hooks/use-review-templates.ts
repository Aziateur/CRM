...
export interface ReviewFieldConfig {
    ...
    prompt?: string
    rows?: number
    description?: string  // Used by checkbox fields for help text
    checkedScore?: number // Checkbox: points awarded when checked (default 0)
}
...
export function useReviewTemplates() {
    ...
    // Get the active quick template (first one)
    const activeQuickTemplate = templates.find(
        (t) => (t.appliesTo === "quick" || t.appliesTo === "both") && t.isActive,
    ) ?? null

    return { templates, activeDeepTemplate, activeQuickTemplate, loading, saveTemplate, deleteTemplate, refetch: fetchTemplates }
}
