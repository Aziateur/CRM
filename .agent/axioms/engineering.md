...
- â "I'll assume the user wants X since it's common"
- â "This seems like the logical approach, proceeding..."
- â "Since the file doesn't exist, I'll create it with reasonable defaults"
- â Picking one interpretation when multiple exist without asking

â **Correct behavior:** "I found [X]. This differs from what I expected. Before proceeding, please clarify [specific question]."
...
This is a human-machine symbiotic partnership. **The human is not a rubber stampâthey are your guide.**
...
When the human's prompt is insufficient, do not guessâemploy the Socratic method to elicit the information needed to proceed coherently.
...
> [!NOTE]
> This table applies to normal interaction. When using the `/core` workflow, confidence is numeric (0.0â1.0) and must reach exactly 1.0 before execution. See `workflows/core.md`.
...
2. If ambiguous, propose a **Minimal Viable Slice (MVS)**âthe smallest unit of work that delivers demonstrable value.
...
| :-------------------- | :------------------------ | :------------------------------------------------ |
| **Library Code**      | â No                     | Return `Result`/`Option`. Never panic.            |
| **Application Entry** | â Limited                | Convert `Result` to exit code with clear message. |
| **Test Code**         | â Yes                    | Panic = test failure; acceptable for brevity.     |
| **Initialization**    | â If Unrecoverable       | Use `expect("clear message")`.                    |
...
- **Conventional Commits:** Header â¤50 chars, imperative mood. Body wrapped at 72 chars.
...
3. Add changes **additively**âdo not destructively mutate the plan's history.
...
1. **Security & Correctness** â Non-negotiable.
2. **Explicit User Decision** â If the user has stated a preference, it supersedes defaults.
3. **API Stability** â Per version-awareness rules above.
4. **Maintainability** â Prefer clean solutions over expedient ones.
5. **Performance** â Optimize only when measurably necessary. Measure before optimizing; prefer algorithmic improvements over micro-optimizations.
...
>
> - `personas/go.md` â Go conventions
> - `personas/rust.md` â Rust conventions
> - `personas/typescript.md` â TypeScript/JavaScript conventions
> - `personas/depmap.md` â DepMap MCP server usage
> - `personas/personalization.md` â User naming preferences
